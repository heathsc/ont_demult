...
impl <'a>fmt::Display for Match<'a> {
	fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result { write!(f, "{}\t{}\t{}", self.site.name, self.site.barcode, self.inner) }
}
...
impl fmt::Display for Location {
	fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result { write!(f, "{}\t*\t{}", self.contig, self.inner) }
}
...
impl fmt::Display for CommonLoc {
	fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
		write!(f, "{}\t{}\t{}\t{}\t{}\t{}\t{}\t{:.4}", self.strand, self.start[0], self.end[0], self.start[1], self.end[1], self.length,
		self.unused, (self.unused as f64) / (self.length as f64))?;
...
impl PafRecord {
	...
	fn from_str_slice(v: &[&str], ctgs: &mut HashSet<Rc<str>>) -> io::Result<Self> {
		...
		if qend <= qstart {
			return Err(Error::new(ErrorKind::Other, format!("Parse error for {}, query start >= query end", target_name)));
		}
...
impl PafRead {
	...
	fn from_str_slice(v: &[&str], ctgs: &mut HashSet<Rc<str>>) -> io::Result<Self> {
		...
		if records[0].qend > qlen {
			return Err(Error::new(ErrorKind::Other, format!("Parse error for {}, query start > query len", qname)));
		}
	...
	fn add_record(&mut self, v: &[&str], ctgs: &mut HashSet<Rc<str>>) -> io::Result<()> {
		...
		if rec.qend > self.qlen {
			return Err(Error::new(ErrorKind::Other, format!("Parse error for {}, query start > query len", self.qname)));
		}
	...
	pub fn find_site<'a, 'b>(&'a self, cut_sites: &'b CutSites, param :&Param) -> Option<FindMatch<'b>> {
		...
		self.records.iter().filter(|r| r.mapq >= threshold && self.qlen < r.target_length + 150).max_by_key(|r| r.matching_bases).map(|r| {
			...
			let mut recs: Vec<_> = self.records.iter().filter(|s| s.target_name == r.target_name && s.strand == r.strand && s.mapq > 0).collect();
			...
			for s in recs.windows(2) {
				if s[0].qend >= s[1].qstart {
					trace!("Read {} mapping to {} overlaps by {} bases - discarded", self.qname, r.target_name, s[0].qend - s[1].qstart + 1);
					skip = true;
					break
				}
			}
			...
			if !skip {
				...
				// Get splits
				let splits: Vec<_> = recs.windows(2).map(|x| {
					if strand == Strand::Plus {
						InteriorSplit { from: x[0].target_end, to: x[1].target_start }
					} else {
						InteriorSplit { from: x[0].target_start, to: x[1].target_end }
					}
				}).collect();

				let cloc = CommonLoc{strand: s.strand, start: [start, spos], end: [end, send], length: self.qlen, unused, splits };
				let check_match = |m| if (unused as f64) / (self.qlen as f64) > param.max_unmatched() {
				...
				Some(match (start_site, end_site, select) {
					(Some(m1), Some(m2), sel) => if m1 == m2 {
						if sel == Select::Xor {
							FindMatch::MatchBoth(Location { contig: s.target_name.clone(), inner: cloc })
						} else {
							check_match(Match { site: m1, inner: cloc })
						}
					} else {
						if sel == Select::Both {
							FindMatch::MisMatch(Location { contig: s.target_name.clone(), inner: cloc })
						} else {
							check_match(Match { site: m1, inner: cloc })
						}
					},
					(Some(_), None, Select::Both) => FindMatch::MatchStart(Location { contig: s.target_name.clone(), inner: cloc}),
					(Some(m), None, _) => check_match(Match { site: m, inner: cloc }),
					(None, Some(m), Select::Either) => check_match(Match { site: m, inner: cloc }),
					(None, Some(_), _) => FindMatch::MatchEnd(Location { contig: s.target_name.clone(), inner: cloc }),
					(None, None, _) => FindMatch::Location(Location { contig: s.target_name.clone(), inner: cloc }),
				})
			} else {
				None
			}
